    ...
    <div className="space-y-4 px-5 py-5">
      ...
      <div>
        ...
        {!loaded ? (
          ...
          <input
            id="beatapi-key"
            type={configured && !editing ? 'text' : 'password'}
            value={configured && !editing ? configuredKeyMask : apiKey}
            onFocus={() => {
            ...
            onBlur={() => {
              if (!apiKey.trim()) setEditing(false);
            }}
            onChange={(event) => {
              setEditing(true);
      ...
      <button
        type="button"
        disabled={!loaded || saving || !apiKey.trim()}
        onClick={() => void save()}
